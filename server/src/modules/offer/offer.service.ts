...
export class OfferService {
  static async list(query: GetOffersQuery) {
    ...
    // Build query
    const offerQuery = OfferModel.find(filter).sort(sort);

    // Apply pagination only if limit > 0
    if (Number(limit) > 0) {
      const skip = (Number(page) - 1) * Number(limit);
      offerQuery.skip(skip).limit(Number(limit));
    }
  ...
  static async update(id: string, data: UpdateOfferBody & { image?: string }) {
    if (!isValidMongoId(id)) {
      throw createError.BadRequest('Invalid offer ID');
    }

    // Get old offer to compare product IDs
    const oldOffer = await OfferModel.findById(id).lean();
    if (!oldOffer) {
      throw createError.NotFound('Offer not found');
    }
    ...
    // remove old image
    if (data.image && oldOffer.image) {
      removeImage(oldOffer.image);
    }
  ...
  static async delete(id: string) {
    ...
    const offer = await OfferModel.findById(id)
      .select('_id applicable_products')
      .lean();
