...
export class CampaignService {
  static async list(query: GetCampaignsQuery) {
    ...
    // Build query
    const campaignQuery = CampaignModel.find(filter).sort(sort);

    // Apply pagination only if limit > 0
    if (Number(limit) > 0) {
      const skip = (Number(page) - 1) * Number(limit);
      campaignQuery.skip(skip).limit(Number(limit));
    }
  ...
  static async create(data: CreateCampaignBody & { image: string }) {
    ...
    // Invalidate list cache (use wildcard pattern to match all list variations)
    await deleteCache(`${CAMPAIGN_RESOURCE}:list:*`);
  ...
  static async update(
    id: string,
    data: UpdateCampaignBody & { image?: string },
  ) {
    if (!isValidMongoId(id)) {
      throw createError.BadRequest('Invalid campaign ID');
    }
    ...
    const oldCampaign = await CampaignModel.findById(id).select('image').lean();
    if (!oldCampaign) {
      throw createError.NotFound('Campaign not found');
    }
    ...
    // remove old image file if image is updated
    if (data.image && oldCampaign.image) {
      removeImage(oldCampaign.image);
    }

    // Invalidate cache - specific campaign and all list variations
    await deleteCache(`${CAMPAIGN_RESOURCE}:${id}:*`);
    await deleteCache(`${CAMPAIGN_RESOURCE}:list:*`);
