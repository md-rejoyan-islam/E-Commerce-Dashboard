...
export class UserService {
  static async list(query: GetUsersQuery) {
    ...
    let userQuery = UserModel.find(filter).sort(sort).select(selectFields);
  ...
  static async create(data: Record<string, unknown>) {
    ...
    // Delete all user-related cache entries using wildcard pattern
    await deleteCache(`${USER_RESOURCE}:*`);
    return { _id: user._id };
  }
