...
export class ProductService {
  ...
  static async list(query: GetProductsQuery) {
    ...
    // Handle is_active as string from query params
  ...
  static async update(id: string, data: Partial<IProduct>) {
    ...
    // remove images from storage if they are deleted
      ...
      for (const variant of product.variants) {
        if (variant.images && variant.images.length > 0) {
          for (const imageUrl of variant.images) {
            // Assuming a utility function deleteImageFromStorage exists
  ...
  static async addReview(
    ...
    await deleteCache('products:reviews:*');
  ...
  // Delete review (Admin - no ownership check)
  static async adminDeleteReview(id: string, reviewId: string) {
    if (!isValidMongoId(id)) {
      throw new Error('Invalid product ID');
    }
  ...
  static async getAllReviews(query: { page?: number; limit?: number }) {
    ...
    // Aggregate to get all reviews with product info
    const reviewsAggregation = await ProductModel.aggregate([
      { $unwind: '$reviews' },
      {
        $lookup: {
          from: 'users',
    ...
    // Get average rating
    const avgRatingResult = await ProductModel.aggregate([
      { $unwind: '$reviews' },
      { $group: { _id: null, avgRating: { $avg: '$reviews.rating' } } },
    ]);
    const averageRating = avgRatingResult[0]?.avgRating || 0;

    // Get this month's reviews count
    const thisMonthResult = await ProductModel.aggregate([
      { $unwind: '$reviews' },
      {
        $match: {
          'reviews.timestamp': { $gte: monthStart, $lte: monthEnd },
        },
      },
      { $count: 'count' },
    ]);
