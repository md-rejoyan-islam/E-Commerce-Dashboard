...
export class OrderService {
  static async list(query: GetOrdersQuery) {
    ...
    if (search) {
      ...
      // If search is a valid number, also search by order id
    ...
    const orderQuery = OrderModel.find(filter)
      ...
      .populate('order_items.product_id', 'name slug variants')
      .sort(sort);

    // Apply pagination only if limit > 0
    if (Number(limit) > 0) {
      const skip = (Number(page) - 1) * Number(limit);
      orderQuery.skip(skip).limit(Number(limit));
    }
  ...
  static async getUserOrders(userId: string, query: GetUserOrdersQuery) {
    ...
    // Build query
    const orderQuery = OrderModel.find({ user_id: userId }).sort(sort);
